...
| | |
|---|---|
| ⚡ **Fast · 快** | ~170ms per query · 单次查询约 170ms |
| 💾 **Lightweight · 轻量** | Fingerprint DB only ~114 MB — runs on any ordinary computer · 指纹库仅约 114 MB，任何普通电脑都能运行 |
| 🚫 **No training / no network · 无需训练、无需联网** | Works on novel, virtual, or known compounds, fully offline · 适用于全新/虚拟/已知化合物，完全离线 |
| 🎯 **4,309 targets · 覆盖 4309 个靶点** | Powered by ChEMBL bioactivity data · 基于 ChEMBL 生物活性数据 |
| 🔬 **SEA+TC dual threshold · 双阈值** | Improved accuracy over classic SEA (Irwin et al. 2018) · 相比经典 SEA 精度更高 |
| 📦 **Zero large downloads · 无需大文件下载** | Extract the bundled 55 MB archive and start — no 29 GB ChEMBL database needed · 解压随仓库附带的 55 MB 数据包即可使用，无需下载 29 GB 数据库 |
...
# 2. Extract pre-built data · 解压预构建数据（只需一次）
tar -xzf compounds-targets-data.tar.gz

# 3. Single compound prediction · 单个化合物预测
