...
The SEA+TC engine requires the ChEMBL SQLite database. Use the bundled download script — it will also auto-generate `target_info.json`:
...
├── chembl_*.db       ← ChEMBL SQLite (downloaded, ~29GB)
├── target_fps.pkl    ← Pre-built fingerprint database (generated, ~114MB)
├── target_info.json  ← Target metadata (auto-generated by download_chembl.py)
